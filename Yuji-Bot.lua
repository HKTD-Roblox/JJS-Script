...
local CFG = {
	...
	combat = {
		...
		blockApproachChance = 0.32,
		rCancelOnBlockChance = 0.72,
	},
	movement = {
		dashCD = 0.70,
		sideDashCD = 0.74,
...
local function tryRCancel()
	...
	if STATE.target.isBlocking or STATE.combat.enemyBlocking then
		if math.random() < CFG.combat.rCancelOnBlockChance then
			if castR() then
				task.wait(0.05)
				backDash()
				return true
...
local function wasRecentHit()
	return (tick() - STATE.combat.lastHitConfirm) < 0.50
end

local function safeM1String()
	if STATE.combat.lock or STATE.combat.comboActive then return end
	if shouldBackoff() then return end
	if STATE.target.dist < CFG.combat.minRange or STATE.target.dist > CFG.combat.attackRange then return end
	if STATE.target.isBlocking or STATE.combat.enemyBlocking then
		if math.random() < 0.6 then
			...
			if goBehind() then
				task.wait(0.05)
				if canSkill(3) then blackFlashProper() end
			end
		end
		return
	end
	...
	task.spawn(function()
		...
		if STATE.target.dist > CFG.combat.attackRange + 1.3 then
			registerWhiff()
			backDash()
			STATE.combat.lock = false
			STATE.combat.comboActive = false
		...
		doM1()
		task.wait(0.08)
		if STATE.target.dist <= 8.2 and canSkill(3) then
			blackFlashProper()
		elseif canSkill(1) and STATE.target.dist <= CFG.skills[1].maxRange then
			castSkill(1)
			task.delay(0.12, function() tryRCancel() end)
		elseif canSkill(2) and STATE.target.dist <= CFG.skills[2].maxRange then
			castSkill(2)
			task.delay(0.12, function() tryRCancel() end)
		end
		task.wait(0.17)
...
local function pressureCombo()
	...
	task.spawn(function()
		...
		doM1()
		task.wait(0.07)
		if STATE.target.dist <= 8.3 and canSkill(3) then
			blackFlashProper()
		elseif canSkill(1) and STATE.target.dist <= CFG.skills[1].maxRange then
			castSkill(1)
		elseif canSkill(2) and STATE.target.dist <= CFG.skills[2].maxRange then
...
local function handleBlock()
	...
	if (atk or sk) and dist <= CFG.defense.blockRange then
		shouldBlock = true
	elseif STATE.target.approaching and dist < 11 and dist > 4 and math.random() < CFG.combat.blockApproachChance then
		shouldBlock = true
	end
	...
	if shouldBlock then
		...
		if STATE.defense.holding then
			local clear = CFG.defense.blockHoldExtra
			if STATE.target.attackClearTime > 0 and (now - STATE.target.attackClearTime) >= clear then
				release(Enum.KeyCode.F, "F")
				STATE.defense.blocking = false
				STATE.defense.holding = false
				STATE.defense.blockEnd = now
...
local function handleRagdoll()
	...
	if isRagdoll(my, hum) then
		...
		local fwd = math.random() > 0.5 and Enum.KeyCode.W or Enum.KeyCode.S
		task.spawn(function()
...
local function updateMovement(dt)
	...
	if hp <= CFG.defense.lowHP then
		...
		if dist < 6.5 then
			faceTarget(true)
			if math.random() < 0.45 and canSkill(4) then
				castSkill(4)
...
local function combatDecision()
	...
	if STATE.target.isBlocking and dist <= 9.5 then
		if math.random() < 0.55 then
			backDash()
			return
		elseif goBehind() then
			task.wait(0.04)
			if canSkill(3) then
				blackFlashProper()
				return
		...
		if canSkill(1) and dist <= CFG.skills[1].maxRange then
			castSkill(1)
			task.delay(0.1, function() tryRCancel() end)
			return
		end
	end
	...
	elseif dist > CFG.combat.attackRange and dist < 13.5 then
		if canSkill(1) and dist <= CFG.skills[1].maxRange then
			castSkill(1)
		end
	end
